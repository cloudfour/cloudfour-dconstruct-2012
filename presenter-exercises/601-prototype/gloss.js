...
$(document).ready(function() {
  var savedGlosses, isSupported, popupWidth;
  popupWidth   = viewport().width - 30;
  popupWidth   = (popupWidth > 500) ? 500 : popupWidth;

  var showSaved = function() {
    var $listWrapper, $list;
    if (!savedGlosses.length) {
      return;
    }
    $list = buildSavedList();
    if (!$('div.saved-glosses').length) {
      $listWrapper = $('<div></div>').addClass('saved-glosses').html('<h3>Saved Items</h3>');
      $listWrapper.append($list);
      $('body').prepend($listWrapper);
      $('.saved-glosses h3').click(function() {
        $(this).parent('div').toggleClass('open').find('dl').slideToggle('fast');
      });
    } else {
      $('div.saved-glosses dl').replaceWith($list);
      if ($('div.saved-glosses').hasClass('open')) {
        $('div.saved-glosses dl').show();
      }
    }
  };
  var buildSavedList = function() {
    var $list, $remove;
    if (!savedGlosses.length) {
      return;
    }
    $list = $('<dl class="saved-glosses"></dl>');

    for(var i = 0; i < savedGlosses.length; i++) {
      var val = savedGlosses[i];
      var $content = $("#" + val).html();
      var $dt = $('<dt></dt>');
      var $dtContent = '';
      $('span[data-gloss="' + val + '"]').each(function() {
        var $definition = $(this).clone();
        $definition.find('sup').remove();
        $dtContent += $definition.html();
      });
      $dt.html($dtContent);
      var $dd = $('<dd></dd>').attr('id', 'saved-' + val).html($content);
      $list.append($dt);
      $list.append($dd);
    }
  ...
  var init = function() {
    isSupported                     = (typeof JSON != 'undefined'
      && typeof JSON.stringify      != 'undefined' 
      && typeof window.localStorage != 'undefined');
    if (isSupported) {
      savedGlosses                  = JSON.parse(localStorage.getItem('glosses')) || [];
      showSaved();
    }
  }();
  var saveGloss = function(val) {
    ...
    savedGlosses[savedGlosses.length] = val;
    localStorage.setItem('glosses', JSON.stringify(savedGlosses));
    $('span[data-gloss="' + val + '"]').addClass('gloss-item-saved');
    showSaved();
  };
  var isSaved = function(gloss) {
    for(var i = 0; i < savedGlosses.length; i++) {
  ...
  var clearGlosses = function() {
    localStorage.removeItem('glosses');
    savedGlosses = [];
    $('span.gloss-item').removeClass('gloss-item-saved');
    $('div.saved-glosses').fadeOut(500, function() { $('div.saved-glosses').remove(); });
  };

  $('.glossary ol li').each(function(index) {
    $('span[data-gloss ="' + $(this).attr('id') + '"]').each(function() {
      var glossId    = $(this).attr('data-gloss');
      $(this).addClass('gloss-item');
      if (isSupported && isSaved(glossId)) {
        $(this).addClass('gloss-item-saved');
      }
      $(this).click(function() {
        var $content, buttons;
        $content     = $("li#" + glossId).html();
        buttons      = [ { label: 'Close' } ];
        if (isSupported && !isSaved(glossId)) {
          buttons[1] = { id: 'save-' + glossId, label: 'Save', val: glossId, btnClass: 'btn-success' };
        }
        new Messi($content, { title: 'Glossary', modal: true, width: popupWidth + 'px', buttons: buttons,
